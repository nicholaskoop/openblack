...
#include "Graphics/Shaders/fs_text.bin.h"
#include "Graphics/Shaders/vs_text.bin.h"
...
namespace
{
	const bgfx::EmbeddedShader s_embeddedShaders[] = { BGFX_EMBEDDED_SHADER(vs_text),
...
TextRenderer::TextRenderer(bgfx::ViewId viewId)
...
void TextRenderer::CreateDeviceObjectsBgfx()
{
	...
	_program = bgfx::createProgram(bgfx::createEmbeddedShader(s_embeddedShaders, type, "vs_text"),
		bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_text"), true);
...
void TextRenderer::DrawText(const Font& font, const std::string& text)
{
	...
	for (const char& c : text) {
		...
		_vertices[vert++] = TextVertex{ static_cast<float>(x+w), static_cast<float>(y), glyph->U1, glyph->V0, r, g, b, 1.0 }; // TR
		_vertices[vert++] = TextVertex{ static_cast<float>(x), static_cast<float>(y+h), glyph->U0, glyph->V1, r, g, b, 1.0 }; // BL
		_vertices[vert++] = TextVertex{ static_cast<float>(x+w), static_cast<float>(y+h), glyph->U1, glyph->V1, r, g, b, 1.0 }; // BR
