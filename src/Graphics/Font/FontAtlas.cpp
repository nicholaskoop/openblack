...
void FontAtlas::build()
{
	...
	_texWidth = (surfaceSqrt >= 4096 * 0.7f) ? 4096 : (surfaceSqrt >= 2048 * 0.7f) ? 2048 : (surfaceSqrt >= 1024 * 0.7f) ? 1024 : 512;
	...
	_texture->Create(_texWidth, _texHeight, 1, graphics::Format::R8, graphics::Wrapping::ClampEdge, _texPixelsAlpha8.get(), _texWidth * _texHeight);
}
	...
	for (auto& shelf : _shelves) {
		if (shelf.Fits(required_width, required_height)) {
			...
			for (unsigned int y = 0; y < glyph.height; y++) {
				memcpy(
					_buffer.get() + ((y_pos + y) * _width + x_pos),
					bitmap + (y * glyph.width),
